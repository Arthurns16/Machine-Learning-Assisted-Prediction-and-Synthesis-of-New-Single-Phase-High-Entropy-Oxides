...
def valence_d_eletrons (elemento):

    L  =  element(elemento)
    valence = L.ec.get_valence()
    valence= str(valence)
    

    indice = valence.find("d")
    
    
    cortado = valence[indice + 1:]
    
    if cortado.find(" ") != -1 :
        fim = cortado.find(" ")
        cortado = cortado[0:fim]
    
    valence_d = 0
    
    if indice != -1 :
        valence_d = float(cortado)

    return valence_d


def vetor_valence_d_eletrons_atomos (linha) :
    ...
    for elemento in atomos:
        try: 
            vetor_propriedade_da_linha.append (float(valence_d_eletrons(elemento)))
        except:
            valor = 0
...
#criar_culunas_excel("valence_d_eletrons")
...
def preencher_linhas (linha_inicial, linha_final) :
    ...
    propriedade_direta = "valence_d_eletrons"
    ...
    while linha < linha_final+1 :
        ...
        vetor = vetor_valence_d_eletrons_atomos (linha)
