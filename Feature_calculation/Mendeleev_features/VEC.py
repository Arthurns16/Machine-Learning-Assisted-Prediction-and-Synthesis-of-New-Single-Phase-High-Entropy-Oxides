...
def VEC (elemento):
    ...
    eletrons_totais = L.ec.ne()
    eletrons_valencia = L.ec.nvalence(L.block, L.period)
    
   


    return eletrons_valencia /  (eletrons_totais )
    


def vetor_VEC_atomos (linha) :
    ...
    for elemento in atomos:
        try: 
            vetor_propriedade_da_linha.append (float(VEC(elemento)))
        except:
            valor = 0
